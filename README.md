...
With **Local first** and a local sidecar, online APIs are still queried **in the background** (when online lookup is enabled) so **Save online segments** and **Sync local → online** can run after playback starts without delaying the first skip dialog. Skip/ask behavior during that session still uses the local sidecar.
